...
awl::backends::x11::pixmap::holder_unique_ptr awl::backends::x11::pixmap::create_from_data(
    awl::backends::x11::window::base const &_window,
    awl::backends::x11::pixmap::dim const &_dim,
    char const *const _data)
{
  Pixmap const result{::XCreateBitmapFromData(
      _window.display().get().get(), _window.get(), _data, _dim.w(), _dim.h())};

  if (result == None)
  {
    throw awl::exception{FCPPT_TEXT("XCreateBitmapFromData failed!")};
  }

  return fcppt::make_unique_ptr<awl::backends::x11::pixmap::holder>(_window.display(), result);
}
