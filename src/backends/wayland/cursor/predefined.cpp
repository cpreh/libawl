    ...
    : impl_{::wl_cursor_theme_get_cursor(_theme.get(), _name.get().c_str())}
{
  ...
  {
    throw awl::exception{
        FCPPT_TEXT("Getting a cursor with name ") + fcppt::from_std_string(_name.get()) +
        FCPPT_TEXT(" failed")};
  }
}

awl::backends::wayland::cursor::predefined::~predefined() = default;

awl::backends::wayland::cursor::optional awl::backends::wayland::cursor::predefined::get() const
{
  return awl::backends::wayland::cursor::optional{impl_};
}
