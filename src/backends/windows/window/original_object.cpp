...
awl::backends::windows::window::original_object::original_object(
    ...
    : awl::backends::windows::window::object(),
      ...
      holder_(awl::backends::windows::window::create(_parameters, _wndclass), _remove_wndclass),
      cursor_(fcppt::optional::map(
          _parameters.cursor(),
          [](fcppt::reference<awl::cursor::object const> const _cursor)
          {
            return fcppt::make_cref(
                fcppt::cast::static_downcast<awl::backends::windows::cursor::object const &>(
                    _cursor.get()));
          })),
      user_data_{
          fcppt::reference_to_base<awl::backends::windows::window::object>(fcppt::make_ref(*this)),
          _events}
{
  fcppt::cast::static_downcast<awl::backends::windows::visual::object const &>(_parameters.visual())
      .apply(this->hwnd());

  awl::backends::windows::window::set_user_data(this->hwnd(), user_data_);
}
...
awl::window::dim awl::backends::windows::window::original_object::size() const
{
  RECT const ret(fcppt::optional::to_exception(
      awl::backends::windows::window::get_client_rect(*this),
      [] {
        return awl::exception{
            FCPPT_TEXT("Can't get the window size because GetWindowRect failed!")};
      }));

  return fcppt::math::dim::to_unsigned(fcppt::math::dim::static_<awl::window::unit, 2>{
      fcppt::cast::size<awl::window::unit>(ret.right - ret.left),
      fcppt::cast::size<awl::window::unit>(ret.bottom - ret.top)});
}

void awl::backends::windows::window::original_object::show()
{
  BOOL const prev_visible(::ShowWindow(this->hwnd(), SW_SHOWDEFAULT));

  if (prev_visible != 0)
    return;

  awl::window::dim const cur_size(this->size());

  awl::backends::windows::post_message(
      this->hwnd(),
      fcppt::strong_typedef_construct_cast<
          awl::backends::windows::message_type,
          fcppt::cast::to_unsigned_fun>(WM_SIZE),
      fcppt::strong_typedef_construct_cast<awl::backends::windows::wparam, fcppt::cast::size_fun>(
          fcppt::cast::to_unsigned(SIZE_RESTORED)),
      awl::backends::windows::lparam{MAKELPARAM(
          fcppt::cast::size<WORD>(cur_size.w()), fcppt::cast::size<WORD>(cur_size.h()))});
}

awl::visual::object const &awl::backends::windows::window::original_object::visual() const
{
  return visual_;
}

HWND awl::backends::windows::window::original_object::hwnd() const { return holder_.get(); }
