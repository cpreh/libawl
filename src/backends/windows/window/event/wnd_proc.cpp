...
LRESULT CALLBACK awl::backends::windows::window::event::wnd_proc(
    HWND const _hwnd, UINT const _message, WPARAM const _wparam, LPARAM const _lparam)
{
  return fcppt::optional::from(
      fcppt::optional::bind(
          awl::backends::windows::window::get_user_data(_hwnd),
          [_message, _wparam, _lparam](
              fcppt::reference<awl::backends::windows::window::user_data const> const _user_data)
          {
            awl::backends::windows::window::event::object const event{
                _user_data.get().window(),
                awl::backends::windows::message_type{_message},
                awl::backends::windows::wparam{_wparam},
                awl::backends::windows::lparam{_lparam}};

            _user_data.get().events().get().push_back(
                awl::backends::windows::window::event::make(event));

            return awl::backends::windows::window::event::handler(event);
          }),
