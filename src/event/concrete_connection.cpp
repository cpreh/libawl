...
awl::event::concrete_connection::concrete_connection(awl::event::connection_function &&_function)
    : awl::event::connection(), function_{std::move(_function)}
{
}
...
awl::event::concrete_connection::~concrete_connection()
{
  ...
  {
    std::cerr << "Unknown error in ~concrete_connection\n";
  }
}
