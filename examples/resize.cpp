...
try
{
  ...
  awl::window::object_unique_ptr const window(
      window_system->create_window(awl::window::parameters(*visual)
                                       .title(FCPPT_TEXT("awltest"))
                                       .class_name(FCPPT_TEXT("awltest"))
                                       .size(awl::window::dim(1024U, 768U))));

  window->show();

  awl::system::event::processor &system_processor(window_system->processor());
             ...
             awl::main::loop_function{
                 [&system_processor](awl::event::base const &_event)
                 {
                   fcppt::optional::maybe_void(
                       ...
                       [&system_processor](auto const &_variant)
                       {
                         fcppt::variant::match(
                             _variant,
                             [](fcppt::reference<awl::window::event::resize const> const _resize) {
                               fcppt::io::cout() << FCPPT_TEXT("Resize: ") << _resize.get().dim()
                                                 << FCPPT_TEXT('\n');
                             },
                             [&system_processor](fcppt::reference<awl::window::event::close const>)
                             {
                               fcppt::io::cout() << FCPPT_TEXT("Close\n");
                             ...
                             {
                               fcppt::io::cout() << FCPPT_TEXT("Destroy\n");
